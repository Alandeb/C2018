...
void menuPrincipal(int cantiUser,int cantiGame){
    eJuego game[cantiGame];
    eCliente users[cantiUser];
    eAlquileres alquiler[cantiUser*cantiGame];
...
int menu()
{
    ...
    printf("12- Salir\n");
    utn_getEntero(&opcion,"\nIngrese opcion: ","\nERROR Ingrese opcion valida: ",1,12);
    return opcion;
}
...
void sortGame(eJuego* game, int cantidad ,int order){
    ...
    if(order==0){
        for(i=0;i<cantidad-1;i++){
            for(j=i+1;j<cantidad;j++){
                if(game[i].importe>game[j].importe&&(game[i].isEmpty==0&&game[j].isEmpty==0)){
                    gameAux=game[i];
                    game[i]=game[j];
                    game[j]=gameAux;
                }else
                    if(game[i].importe==game[j].importe){
                        if((stricmp(game[i].descripcion,game[j].descripcion))>0&&(game[i].isEmpty==0&&game[j].isEmpty==0)){
                            gameAux=game[i];
                            game[i]=game[j];
                            game[j]=gameAux;
                        }
